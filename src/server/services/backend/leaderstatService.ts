...
export class LeaderstatService implements OnStart {
    ...
    onStart() {
        ...
        this.playtimeService.playtimeUpdated.Connect((player, formattedPlaytime) => {
            const leaderstats = player.FindFirstChild("leaderstats");
            if (leaderstats) {
                const playtimeValue = leaderstats.FindFirstChild("Playtime") as StringValue | undefined;
                if (playtimeValue) {
                    playtimeValue.Value = formattedPlaytime;
                }
            }
        });
    ...
    private createLeaderstats(player: Player, profile: any) {
        ...
        playtime.Value = this.playtimeService.getCurrentPlaytime(player);
