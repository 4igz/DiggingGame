...
interface ServerToClientEvents {
	...
	createWaypointVisualization: (targetPosition: Vector3, detectorName: keyof typeof metalDetectorConfig) => void;
...
interface ClientToServerFunctions {
	...
	getLastDailyClaimTime: () => number;
	getDailyStreak: () => number;
	getInventorySize: () => number;
	claimPlaytimeReward: (rewardIndex: number) => boolean;
	getQuestProgress: () => Map<keyof typeof questConfig, QuestProgress>;
